...
/**
 * 复制的AbstractQueuedSynchronized类，对这个类做一些注解
 * @author lh
 *
 */
public abstract class AbstractQueuedSynchronizer1
    ...
    private Node enq(final Node node) {
        for (;;) {
            ...
            if (t == null) { // Must initialize
                ...
                node.prev = t;
                if (compareAndSetTail(t, node)) {//这里初始化完之后，实际上head节点是一个空的node， 没有任何线程
                    t.next = node;
                    return t;
                }
            }
    ...
    private void unparkSuccessor(Node node) {
         ...
         */
        Node s = node.next;
        if (s == null || s.waitStatus > 0) {//如果下一个节点为空，或者状态是取消，则从后往前找有效节点
            s = null;
    ...
    final boolean acquireQueued(final Node node, int arg) {
        ...
        try {
            ...
            for (;;) {
                ...
                if (p == head && tryAcquire(arg)) {//如果上一个节点是head，说明这个node排在第一位，那么执行tryAcquire()操作
                    setHead(node);
                    p.next = null; // help GC
                    failed = false;
                    return interrupted;
                }
                //不是排在第一位，并且尝试获取锁失败，则进入这一步
              //如果前置节点已经有状态了，则直接返回,并且执行park()操作，把当前线程挂起;如果前置节点没有状态则当前线程不用挂起
    ...
    public final boolean hasQueuedPredecessors() {
        ...
        Node h = head;
        Node s;
        return h != t &&  //只要head后边的第一个节点是这个线程，那么就属于没有排队的情况，因为他是队列第一位
